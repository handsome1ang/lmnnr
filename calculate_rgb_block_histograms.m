    ...
    for bs = 1:length(block_sizes_y)
        for overlap = [true]%[true false]
            for nb = 1:length(histogram_sizes)
                ...
                bin_boundaries = linspace(0,256,num_bins+1);
                ...
                for set_selection = ['a', 'b']
                    ...
                    switch set_selection
                        case 'a'
                            if c_cor_flag, 
                                file_name = sprintf('cam_a%s.mat', c_cor);
                            else
                                file_name = 'cam_a.mat'; 
                            end                            
                            
                            set_to_process = fullfile(params.data_dir, file_name);
                            % set_to_process = params.all_viper_cam_a;
                            
                            images = load(set_to_process);
                            images = images.images_a;
                            target_file = sprintf('rgb_cam_a%s', suffix);
                        case 'b'
                            if c_cor_flag, 
                                file_name = sprintf('cam_b%s.mat', c_cor);
                            else
                                file_name = 'cam_b.mat'; 
                            end                            
                            
                            set_to_process = fullfile(params.data_dir, file_name);                            
                            % set_to_process = params.all_viper_cam_b;
                            
                            images = load(set_to_process);
                            images = images.images_b;
                            target_file = sprintf('rgb_cam_b%s', suffix);
                    end
                    
                    fprintf('Source: %s\n', set_to_process);
                    fprintf('Target: %s\n', target_file);
                    
                    %set_to_process = params.image_list_a;
                    %target_path = fullfile(params.block_hist_dir, 'rgb_cam_a.mat');
