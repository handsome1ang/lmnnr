...
while diffloss > tol
    ...
    for i=1:num_images
		...
		% find the target 
		target_idx = find( (labels==labels(i)) & not_me)  ;

        % distance to target
        dist_to_target = dists(i,target_idx);

        tmp = data(i,:)' - data(target_idx,:)';
        trgt = tmp*tmp';
        grdnt = grdnt + (1-mu)*trgt;
        loss(iter) = loss(iter) + dist_to_target;
        
        % find impostors (those that violate the hinge loss)
        impostors = find((dists(i,:) <= dist_to_target + 1) & not_me);
		...
		grdnt = grdnt + mu*(num_impostors * trgt - impst);

		loss(iter) = loss(iter) + num_impostors*(1 + dist_to_target) - trace(M*impst);
    ...
    while true 
        ...
        if any(diag(D)<0) || any(~isreal(D(:)))
            positive_const = positive_const + 1e-12;
        else 
            break;
        end
